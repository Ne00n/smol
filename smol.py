...
url=sys.argv[1];h=[];H={"Content-Type":"application/json","session_id":uuid4().hex}
b=dict(model="model",input=h,tools=[dict(type="function",name="sh",description="Run a shell command and return its output.",parameters=dict(type="object",properties=dict(command=dict(type="string")),required=["command"]))])
while True:
